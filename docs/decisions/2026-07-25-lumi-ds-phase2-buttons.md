...
- `scripts/probe-ds-buttons.mjs` — computed-style probe over 14 routes × EN·VI × dark·light × 390/768/1440px. **500 button states measured, all ≥4.5:1 WCAG against their composited backdrop, all ≥44px, all pill, none overflowing the viewport.** Colours are sampled through a canvas so `display-p3`, `oklab`, and `color-mix()` resolve to the sRGB bytes Lighthouse reads, and styles are allowed to settle first — reading mid-transition reports interpolated colours and produces phantom failures.
