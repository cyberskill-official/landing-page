...
# FR-BIZ-016: CyberOS central data platform for landing leads and content

## 0. Why (evidence)

The operator has decided CyberOS is the centralized system of record for landing-page data (leads, blogs/notes, and related publishable content). FR-BIZ-002 only wires lead intake and explicitly leaves “Building the CyberOS side” out of scope. Without a platform FR, content remains stuck in `lib/content/*` and leads remain split between email/Slack, local Prisma, and an optional webhook.
...
- 1.4 PDPL retention, erasure, and processor roles for leads and transcripts held in CyberOS SHALL be documented before production traffic is pointed at it (pairs with FR-BIZ-012).

## 2. Acceptance criteria

- [ ] AC for 1.1 - lead-intake rejects unauthenticated POSTs and accepts a valid payload - evidence: request log / FR-BIZ-003
...
- [ ] AC for 1.4 - PDPL note lists CyberOS as processor for leads - evidence: FR-BIZ-012 update or dedicated doc

## 3. Edge cases

- CyberOS partial outage: landing must still accept leads (FR-OPS-010 / dual-write).
- Content API schema change must be versioned so the landing build does not silently empty pages.

## 4. Out of scope / non-goals

- Implementing the Next.js read adapter (FR-OPS-019).
- Migrating historical leads (FR-OPS-020).
