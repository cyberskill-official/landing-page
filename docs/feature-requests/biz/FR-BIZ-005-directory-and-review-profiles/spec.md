...
- 1.1 Profiles SHALL be created and fully populated on Clutch, GoodFirms, DesignRush, and the local employer boards (ITviec / TopDev), using the canonical entity sentence, the same logo, the identical NAP, and site links carrying the UTM standard (FR-OPS-011).
...
- 1.4 At least two verified reviews SHALL be live before this FR is accepted.

## 2. Acceptance criteria

- [ ] AC for 1.1 - each profile is live, populated and consistent with the register (FR-BIZ-004) - evidence: URLs recorded
...
- Clutch verification involves a client interview - the client must consent to that, not only to a quote (FR-BIZ-006).
- A directory that publishes wrong figures (employee count, rate) must be corrected, not left.

## 4. Out of scope / non-goals

- Displaying the badges on site (FR-SEO-015, FR-CMS-014).
