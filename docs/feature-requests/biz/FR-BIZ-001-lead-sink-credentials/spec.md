...
# FR-BIZ-001: Configure the lead sinks: Resend domain, API key, Slack webhook

## 0. Why (evidence)

app/api/lead/route.ts fans out to Resend, Slack and a CRM webhook, all of which no-op until their env vars exist - so in
production a lead exists only as a console line in the Vercel function logs. Every conversion FR in this backlog depends on
...
- Code changes to the route (FR-OPS-010).
