...
- 1.1 A one-page profile per locale SHALL be generated from the site's **publishable content read model** (today: `lib/content/*` + commercial policy; after FR-OPS-019: CyberOS with git fallback) so the PDF cannot invent facts absent from the live site, and stored under public/downloads/.
