...
FR-CTA-002 is ship-grade. Calendly embed — Buy track: 3-step Buy form with What kind of help → details → slot picker. Consumes FR-CTA-001 modal. Depends on FR-CTA-001 + FR-WEB-006.

## §2 — Round-1/2 findings (resolved)

### ISS-70001 — Implementation contract underspecified
- **severity:** error
- **rule_id:** API/contract precision
- **status:** RESOLVED — §1 ties to canonical anchor FRs (FR-CTA-001 + FR-WEB-006); type-safe surface per CONTRACT precision rule.

### ISS-70002 — A11y / form-error / aria-live handling
- **severity:** error
- **rule_id:** a11y
- **status:** RESOLVED — Form-error announcements via aria-live=polite; FR-A11Y-010 + FR-A11Y-008 focus-ring discipline.

### ISS-70003 — Rate-limit + abuse protection
- **severity:** warning
- **rule_id:** governance
- **status:** RESOLVED — /api/lead has rate-limit + reCAPTCHA-style bot guard; FR-CTA-006 §1 explicit.

### ISS-70004 — Locale + hreflang correctness
- **severity:** warning
...
### ISS-70005 — Form session draft expiry edge case (deploy mid-session)
- **severity:** warning
- **rule_id:** API/contract precision
...
### ISS-70006 — Honeypot field a11y (must not be announced to AT)
- **severity:** warning
- **rule_id:** a11y
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
### ISS-70007 — Multi-step form state across browser back-button
- **severity:** warning
- **rule_id:** API/contract precision
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
Batch 11 item 1 of 12.

*End of FR-CTA-002 audit.*
