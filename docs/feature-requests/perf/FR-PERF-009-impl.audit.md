...
FR-PERF-009 is ship-grade. navigator.deviceMemory < 4 → LOD-1 + no post-processing + dpr=[1,1]

## §2 — Round-1/2 findings (resolved)

### ISS-80071 — Implementation contract underspecified
- **severity:** error
...
### ISS-80075 — Vietnamese network conditions not in test matrix
- **severity:** warning
...
### ISS-80076 — Sampling bias on low-traffic site (SMB)
- **severity:** info
...
### ISS-80077 — INP vs FID transition for older browsers
- **severity:** info
- **rule_id:** API/contract precision
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
Batch 12 item 8 of 29.

*End of FR-PERF-009 audit.*
