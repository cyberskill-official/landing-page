...
# FR-PERF-013: Measure and gate real Core Web Vitals: mobile Lighthouse assertions plus field data
...
kept in the green", so measuring and holding them is a credibility matter, not only an engineering one. FR-PERF-002 and
FR-PERF-006 shipped Lighthouse CI and Speed Insights; this FR raises them to an asserted, regression-proof floor.
...
- Low traffic means sparse CrUX - the FR must not assume CrUX exists; Speed Insights is the primary source.
...
- Fixing the metrics (FR-PERF-007/008/012).
