...
FR-PERF-002 is ship-grade. Lumi LOD-1 variant (~8k tri) for distance + low-memory paths

## §2 — Round-1/2 findings (resolved)

### ISS-80001 — Implementation contract underspecified
- **severity:** error
...
### ISS-80005 — Vietnamese network conditions not in test matrix
- **severity:** warning
...
### ISS-80006 — Sampling bias on low-traffic site (SMB)
- **severity:** info
...
### ISS-80007 — INP vs FID transition for older browsers
- **severity:** info
- **rule_id:** API/contract precision
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
Batch 12 item 1 of 29.

*End of FR-PERF-002 audit.*
