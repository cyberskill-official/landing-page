...
You are about to run `ship-feature-requests` against this backlog. This file is the
five-minute brief. It exists because the queue, the contract and the gates in this
repo have a few sharp edges that will otherwise cost you a wasted FR.
...
> the first `ready_to_implement` FR with `owner: agent` whose every `depends_on` is `done`.

Three things follow, and all three are deliberate:

- **`owner: human` and `owner: mixed` FRs are not yours.** They hold accounts,
  credentials, client permissions and commercial decisions (the `BIZ` module, plus a
  few content FRs). If you find yourself needing a price range, a testimonial, a client
  name, a metric or an API key - stop. That input is an FR, it is owned by Stephen, and
  the correct behaviour is to build the mechanism so it degrades gracefully without the
  input, never to invent the input. Every FR in this state already has a clause saying
  what the absent-input state must do, and an AC that tests it.
- **No agent-owned FR depends on a human-owned FR.** That was checked (2026-07-11:
  0 stalls, 0 cycles). If you ever add such a dependency you have created a permanent
  stall - re-scope instead.
- **`on_hold` and `closed` FRs are skipped.** Do not resurrect one without an operator
  flip.

## 2. The contract

`.cyberos/cuo/templates/FR-TEMPLATE.md`, and `docs/adrs/ADR-001-fr-contract.md` explains
why it is that one and not the plugin rubric's `feature_request@1`. Five sections:
...
4. **Out of scope** - do not drift into these. They are usually another FR.
5. **Protected invariants** - the things a green gate must never be bought with. If a
   test only passes by weakening one of these, you have a fork: park the FR, record it,
   move on. Do not weaken the invariant.

**Do NOT run the CyberOS plugin's `/feature-request-audit` against these FRs.** It
audits a different contract and fails ~12 rules on every FR here, including shipped
...
- **The performance budget is law.** `lighthouse/budget.json` and
  `scripts/check-asset-size.mjs`. If an FR cannot fit the budget, that is a fork -
...
FR touches UI, and several FRs cite them explicitly:
...
- One FR per branch: `auto/<fr-id-slug>`. Conventional commit with the FR id in
  brackets: `feat(seo): restore word spacing in kinetic headings [FR-SEO-010]`.
...
- **HITL is required and you do not cross it.** Two transitions need a recorded human
  verdict: `reviewing -> ready_to_test` and `testing -> done`. Bring the FR up to each
  gate with the evidence and halt. You never set an FR to `done` yourself
  (`.cyberos/cuo/STATUS-REFERENCE.md` §1.4).
- Update the FR's `status:` and the BACKLOG row at every phase transition. `BACKLOG.md`
  is regenerated from the FR frontmatter - the frontmatter is the source of truth.

## 6. Where the work came from

`docs/audits/2026-07-11/` holds the three audits every `improvement` FR traces to
...
`MIGRATION-MAP.md` maps every finding to its FR, and lists the findings that were
deliberately NOT turned into FRs and why. If a clause looks arbitrary, its evidence is
...
1. **Prove the lead pipeline exists** - FR-OPS-010, then FR-BIZ-001/003 (Stephen). Every
   conversion FR below is worthless while a submitted lead lands in a Vercel log line.
2. **Fix mobile** - FR-PERF-007 (CLS 0.431; audit B says this one metric is worth 20-25
   performance points), FR-PERF-008 (JS weight), FR-WEB-011 (the 0.84 s root redirect).
3. **Get proof on the page** - FR-CMS-011/012/013 and FR-WEB-012 build the mechanisms;
   FR-BIZ-006 (Stephen) supplies the permissions that fill them.
4. **Close the cheap search and answer-engine gaps** - FR-SEO-010/011/012/017/019.
