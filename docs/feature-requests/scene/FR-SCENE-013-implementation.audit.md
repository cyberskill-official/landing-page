...
FR-SCENE-013 is ship-grade. Scene 1 Origin implementation: coil_idle anim + idea-spark shader + typed captions. Consumes FR-SCENE-002 + FR-CHAR-011 into a user-visible scene per FR-WEB-003 SceneTunnel pattern.

## §2 — Round-1/2 findings (resolved)

### ISS-60131 — Implementation contract underspecified
- **severity:** error
...
### ISS-60135 — Scene transition timing accumulates float-error over long scrolls
- **severity:** info
- **rule_id:** API/contract precision
...
### ISS-60136 — Camera matrix not deterministic on resize during scene transition
- **severity:** warning
- **rule_id:** API/contract precision
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
### ISS-60137 — Reduced-motion fallback content parity with cinematic
- **severity:** warning
- **rule_id:** a11y
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
Batch 10 item 1 of 12. Next: FR-SCENE-014.

*End of FR-SCENE-013 audit.*
