...
✓ scripts/__tests__/gltf-pipeline.test.mjs (7 tests) 929ms
Test Files  1 passed (1)
Tests  7 passed (7)

node scripts/gltf-pipeline.mjs
usage: gltf-pipeline.mjs <input.glb> <output.glb> [--dry-run] [--verbose]

node scripts/gltf-pipeline.mjs assets-built/raw/lumi-nonla.raw.glb /tmp/lumi-nonla.audit.glb --dry-run
"strategy": "meshopt"
"beforeBytes": 25836
"afterBytes": 6052
"passedEarlyWarn": true
"extensionsUsed": ["EXT_meshopt_compression", "KHR_mesh_quantization"]
```

Coverage gate note: `./node_modules/.bin/vitest run scripts/__tests__/gltf-pipeline.test.mjs --coverage` is blocked before execution because `@vitest/coverage-v8` is not installed, and no existing `c8`/`nyc` binary is present. No new dependency was introduced during strict audit.
