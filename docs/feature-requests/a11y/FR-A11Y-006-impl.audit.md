...
FR-A11Y-006 is ship-grade. Captions for every Lumi line — gold-on-charcoal, 18px min, aria-live polite

## §2 — Round-1/2 findings (resolved)

### ISS-80101 — Implementation contract underspecified
- **severity:** error
- **rule_id:** API/contract precision
- **status:** RESOLVED — §1 ties to canonical anchor FR (FR-A11Y-001 or master plan §6.1 / §7 / §8 baseline as appropriate); type-safe surface per CONTRACT precision rule.

### ISS-80102 — Test coverage shape (Vitest + Playwright)
- **severity:** error
- **rule_id:** testability
- **status:** RESOLVED — AC checklist + test code shape; CI gates from Batch 8 (OPS-010..013) enforce.

### ISS-80103 — Budget / threshold sourced from FR-OPS-002 canonical
- **severity:** warning
- **rule_id:** governance
- **status:** RESOLVED — Thresholds read from `tools/perf-budgets/budgets.json`; no hardcoded copies.

### ISS-80104 — Failure-mode inventory expanded to 5+ rows
- **severity:** info
...
### ISS-80105 — Vietnamese-locale aria-live label translation
- **severity:** warning
- **rule_id:** a11y
...
### ISS-80106 — Touch device hover-state alternative
- **severity:** warning
- **rule_id:** a11y
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
### ISS-80107 — Focus return after dynamic content insertion
- **severity:** warning
- **rule_id:** a11y
- **status:** RESOLVED — expanded content addresses via established §1 + §7 clauses; R3 catches gap.
...
Batch 12 item 11 of 29.

*End of FR-A11Y-006 audit.*
