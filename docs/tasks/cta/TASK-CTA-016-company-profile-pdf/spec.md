...
# TASK-CTA-016: Company profile one-pager PDF, EN and VN
...
- 1.1 A one-page profile per locale SHALL be generated from the site's **publishable content read model** (today: `lib/content/*` + commercial policy; after TASK-OPS-019: CyberOS with git fallback) so the PDF cannot invent facts absent from the live site, and stored under public/downloads/.
...
- [x] AC for 1.1 - both PDFs build and are under 1 MB - test: `assets/profile-pdf-size`
- [x] AC for 1.2 - links render and downloads emit the event - test: `analytics/both-lead-paths`
- [x] AC for 1.3 - every fact in the PDF traces to the publishable content read model / content module - test: `content/profile-pdf-parity`
