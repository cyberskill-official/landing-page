...
priority: MUST
owner: agent
depends_on: [TASK-BIZ-016]
routed_back_count: 0
awh: N/A
traces_to: [docs/architecture/cyberos-central-data-source, TASK-CMS-001, TASK-CMS-007]
---

# TASK-OPS-019: Landing content read model from CyberOS (with git fallback)
...
- Stale cache after publish: document revalidation strategy (ISR or on-demand) without blocking this task's contract.
- Draft entities must never appear on public routes.

## 4. Out of scope / non-goals

- Migrating every historical post (TASK-CMS-021).
- Lead dual-write (TASK-OPS-020).
