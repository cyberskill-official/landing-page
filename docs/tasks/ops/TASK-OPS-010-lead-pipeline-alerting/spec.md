...
priority: MUST
owner: agent
depends_on: [TASK-OPS-006]
routed_back_count: 0
awh: N/A
traces_to: [growth/LEAD-03, growth/LEAD-04, audit-A/section-9]
---

# TASK-OPS-010: Alert when every lead sink fails, and prove the pipeline weekly with a synthetic lead
...
- 1.2 When every configured sink fails, or when zero sinks are configured in production, the route SHALL report the failure to the error tracker (TASK-OPS-006) with the per-sink reasons, while keeping the ok:true response to the visitor.
...
- The synthetic lead must not trigger the auto-acknowledgement email (TASK-CTA-011) to a real address.
- Rate limiting must not block the weekly synthetic POST.

## 4. Out of scope / non-goals

- Deploying the CyberOS intake (TASK-BIZ-002).
