...
priority: SHOULD
owner: agent
depends_on: [TASK-OPS-004]
routed_back_count: 0
awh: N/A
traces_to: [research-doc/section-O, growth/LEAD-02, audit-A/section-9]
---

# TASK-OPS-005: Lead and transcript datastore behind /api/lead and /api/genie
...
- 1.2 The same datastore SHALL persist Genie chat transcripts received by `/api/genie`, linked to a lead where one exists (TASK-CHAR-028).
...
- 1.5 Without a configured datastore the persistence layer SHALL no-op safely behind the TASK-OPS-010 alert rather than failing the visitor's request; provisioning the datastore is the operator's step.
...
- The store being down must not fail the visitor's submission - persistence is best-effort behind the alert (TASK-OPS-010).
- A transcript can contain personal data the visitor typed about a third party.

## 4. Out of scope / non-goals

- Provisioning the database (operator step, via TASK-BIZ-002 or a managed DB).
