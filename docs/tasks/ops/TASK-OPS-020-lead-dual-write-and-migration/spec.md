...
priority: MUST
owner: mixed
depends_on: [TASK-BIZ-002, TASK-OPS-005]
routed_back_count: 0
awh: N/A
traces_to: [docs/architecture/cyberos-central-data-source, growth/LEAD-02, TASK-BIZ-009]
---

# TASK-OPS-020: Lead dual-write to CyberOS and historical migration

## 0. Why (evidence)

`/api/lead` already best-effort fans out to CyberOS when `LEAD_CRM_WEBHOOK_URL` is set and also persists via `lib/db` (InMemory/Prisma). Without an explicit dual-write + migration task, historical local leads never enter CyberOS and operators may treat Prisma as a second CRM.
...
- 1.2 CyberOS failure SHALL NOT fail the visitor response when another ack path succeeded; TASK-OPS-010 style alerts SHALL fire.
...
- 1.4 Intent, source, locale, and UTM fields SHALL survive dual-write unchanged (TASK-OPS-011 / TASK-CTA-006 mapping).
...
- Building CyberOS UI for SLA (TASK-BIZ-009).
- Content read model (TASK-OPS-019).
