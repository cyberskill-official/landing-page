...
# TASK-OPS-012: Cookieless session replay (Microsoft Clarity), env-gated
...
- 1.1 The Clarity snippet SHALL load only in production, only when NEXT_PUBLIC_CLARITY_ID is set, only in cookieless mode, and only after the consent gate (TASK-OPS-013) allows it.
