...
# TASK-OPS-004: Env and secret management across production and preview, with separate keys
...
- 1.5 The code SHALL read every provider key from server env and SHALL fail closed (a named error, never a silent no-op on a write path) when a required production key is absent; provisioning the separate production and preview key VALUES is TASK-BIZ-001.
...
- Obtaining the credentials themselves (TASK-BIZ-001).
