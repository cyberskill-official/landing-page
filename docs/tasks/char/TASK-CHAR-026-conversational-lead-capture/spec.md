...
# TASK-CHAR-026: Conversational lead-capture sequence, value-first, with ICP qualification and a LEAD_CAPTURED state
...
- 1.3 Lumi SHALL qualify the enquiry against the ICP recorded in the commercial policy (TASK-BIZ-013) and SHALL adapt its follow-ups to the answers rather than reading a fixed script. Where no ICP is recorded, Lumi SHALL collect the lead without qualifying it and SHALL NOT invent criteria.
...
- A visitor who gives an email and nothing else - partial capture must still be useful (TASK-CHAR-028).
- A hostile visitor probing for the system prompt mid-flow (TASK-CHAR-029).
- A Vietnamese conversation must qualify identically to an English one.

## 4. Out of scope / non-goals

- The CRM/Slack handoff (TASK-CHAR-027).
- Transcript persistence (TASK-CHAR-028).
