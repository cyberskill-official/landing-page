...
priority: SHOULD
owner: agent
depends_on: [TASK-OPS-005, TASK-OPS-013]
routed_back_count: 0
awh: N/A
traces_to: [research-doc/section-I, research-doc/section-G, audit-A/section-8]
---

# TASK-CHAR-028: Persist chat transcripts and lead records server-side, including partial conversations
...
- 1.1 The server SHALL persist chat transcripts and lead records to the datastore (TASK-OPS-005), keyed by session so a conversation can be reconstructed.
...
- 1.5 A visitor SHALL be told, before the first message, that the conversation is processed by Anthropic and stored, per TASK-OPS-013.
...
- The store being unavailable must not break the chat - persistence is best-effort, but a total failure must alert (TASK-OPS-010).
- A deletion request from a visitor must remove the transcript (PDPL right to erasure).

## 4. Out of scope / non-goals

- Provisioning the datastore (TASK-OPS-005).
- The formal PDPL review (TASK-BIZ-012).
