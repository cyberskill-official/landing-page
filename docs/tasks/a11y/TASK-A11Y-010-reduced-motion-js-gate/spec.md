...
# TASK-A11Y-010: Honour prefers-reduced-motion in JavaScript: gate the WebGL scene and particle system
...
- 1.3 The user-facing motion toggle (TASK-A11Y-001) SHALL be authoritative over the same sources: when motion is off, no rAF loop, no canvas tick, no particle emission.
...
- forced-colors mode combined with reduced motion (TASK-A11Y-007 contract).
